            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <button class="btn btn-primary" onclick="modalBasicFormCreate()"><span class="tf-icons bx bx-plus-circle"></span> Tambah Menu</button>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive text-nowrap">
                            <table class="table table-hover">
                                ...
                                <tbody class="table-border-bottom-0">
                                    <?php $counter = (isset($_GET['page']) && $_GET['page'] > 1) ? (10 * $_GET['page'] - 10) : 1; ?>
                                    <?php foreach ($menus as $index => $item) : ?>
                                        <tr class="<?= $item['is_best_seller'] == 1 ? 'bg-best-seller' : '' ?>">
                                            ...
                                            <td class="d-flex align-items-center gap-2">
                                                <img src="/img/avatars/1.jpg" alt="<?= $item['name'] ?>" class="w-10 rounded">
                                                <?= $item['name'] ?>
                                            </td>
                                            ...
                                            <td class="text-center">
                                                <div class="d-flex gap-2">
                                                    ...
                                                    <a class="text-black" href="<?= site_url('master/menus/delete/' . $item['id']) ?>" onclick="return confirm('Anda yakin ingin menghapus menu ini?')" data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="top" data-bs-html="true" title="Hapus"><i class="bx bx-xs bx-trash"></i></a>
                                                </div>
                                            </td>
                                        </tr>
                                        <?php $counter++; ?>
...
<script>
    const modalBasicFormCreate = () => {
        $('#form-modal').attr('action', '/master/menus')
    ...
    $('.modal-basic-edit').click(function(e) {
        ...
        const id = $(this).data('id'),
            name = $(this).data('name')
        $('#form-modal').attr('action', '/master/menus/update/' + id)
